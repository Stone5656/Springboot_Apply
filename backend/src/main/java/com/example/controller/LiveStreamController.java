...
import com.example.dto.live_stream.*;
...
public class LiveStreamController {
    ...
    public ResponseEntity<LiveStreamResponseDTO> createLiveStream(
            ...
            @Parameter(hidden = true, description = "現在認証されているユーザー") @RequestAttribute("currentUser") User currentUser) {
        return ResponseEntity.ok(liveStreamService.createLiveStream(request, currentUser));
    }

    @Operation(summary = "ライブ配信詳細取得", description = "指定されたIDのライブ配信情報を取得します。公開・非公開問わず取得可能です。")
    @GetMapping("/{id}")
    public ResponseEntity<LiveStreamResponseDTO> getLiveStreamById(
            @Parameter(description = "ライブ配信ID") @PathVariable Long id) {
    ...
    @Operation(summary = "ライブ配信削除", description = "指定されたライブ配信を削除します。認証が必要です。")
    @SecurityRequirement(name = "bearerAuth")
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteLiveStream(@Parameter(description = "ライブ配信ID") @PathVariable Long id) {
    ...
    @Operation(summary = "ステータス別ライブ一覧", description = "配信ステータスに基づく配信一覧をページング付きで取得します。")
    @GetMapping("/status")
    ...
    @Operation(summary = "複数ステータスのライブ一覧", description = "複数の配信ステータスでフィルタリングした配信一覧を取得します。")
    @GetMapping("/statuses")
    ...
    @Operation(summary = "タイトル検索付きライブ一覧", description = "タイトル部分一致とステータス指定で配信を検索します。")
    @GetMapping("/search")
