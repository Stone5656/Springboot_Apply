...
class CategoryControllerSecurityTest extends MvcTestBase {
  ...
  void search_categories_public(String who, Supplier<RequestPostProcessor> auth, boolean allowed)
      throws Exception {
    var page = new PageImpl<>(
        List.of(CategoryResponseDTO.builder().id(UUID.randomUUID()).name("Tech").build()),
        PageRequest.of(0, 20), 1);

    given(
        categoryService.searchCategories(any(CategorySearchRequestDTO.class), any(Pageable.class)))
            .willReturn(page);

    int s =
        mvc.perform(get("/api/categories").with(auth.get())).andReturn().getResponse().getStatus();
  ...
  @MethodSource("allRolesInclAnon")
  void get_category_public(String who, Supplier<RequestPostProcessor> auth, boolean allowed)
      throws Exception {
    ...
    int s = mvc.perform(get("/api/categories/{id}", id).with(auth.get())).andReturn().getResponse()
        .getStatus();
  ...
  void add_video_categories(String who, Supplier<RequestPostProcessor> auth) throws Exception {
    ...
    int s = mvc
        .perform(post("/api/videos/{videoId}/categories", videoId).with(auth.get())
  ...
  void replace_video_categories(String who, Supplier<RequestPostProcessor> auth) throws Exception {
    ...
    willDoNothing().given(categoryService).replaceVideoCategory(eq(videoId), anyList());

    int s = mvc
        .perform(put("/api/videos/{videoId}/categories", videoId).with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf())
            .contentType(MediaType.APPLICATION_JSON).content(body))
        .andReturn().getResponse().getStatus();
  ...
  void remove_video_category(String who, Supplier<RequestPostProcessor> auth) throws Exception {
    ...
    int s = mvc
        .perform(delete("/api/videos/{videoId}/categories/{categoryId}", videoId, categoryId)
  ...
  void add_livestream_categories(String who, Supplier<RequestPostProcessor> auth) throws Exception {
    ...
    int s = mvc
        .perform(post("/api/live-streams/{liveStreamId}/categories", liveStreamId).with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf())
            .contentType(MediaType.APPLICATION_JSON).content(body))
        .andReturn().getResponse().getStatus();
  ...
  @MethodSource("authedCases")
  void replace_livestream_categories(String who, Supplier<RequestPostProcessor> auth)
      throws Exception {
    var liveStreamId = UUID.randomUUID();
    var body = om.writeValueAsString(List.of(UUID.randomUUID()));

    willDoNothing().given(categoryService).replaceLiveStreamCategory(eq(liveStreamId), anyList());

    int s = mvc
        .perform(put("/api/live-streams/{liveStreamId}/categories", liveStreamId).with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf())
            .contentType(MediaType.APPLICATION_JSON).content(body))
        .andReturn().getResponse().getStatus();
  ...
  @ParameterizedTest(
      name = "DELETE /api/live-streams/'{'liveStreamId'}'/categories/'{'categoryId'}' - {0}")
  @MethodSource("authedCases")
  void remove_livestream_category(String who, Supplier<RequestPostProcessor> auth)
      throws Exception {
    var liveStreamId = UUID.randomUUID();
    var categoryId = UUID.randomUUID();
    ...
    int s = mvc
        .perform(delete("/api/live-streams/{liveStreamId}/categories/{categoryId}", liveStreamId,
            categoryId).with(auth.get()).with(SecurityMockMvcRequestPostProcessors.csrf()))
        .andReturn().getResponse().getStatus();
  ...
  @MethodSource("authedCases")
  void create_category_role_matrix(String who, Supplier<RequestPostProcessor> auth)
      throws Exception {
    ...
    int s = mvc
        .perform(post("/api/categories").with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf())
  ...
  @MethodSource("authedCases")
  void update_category_role_matrix(String who, Supplier<RequestPostProcessor> auth)
      throws Exception {
    UUID id = UUID.randomUUID();
    var req = CategoryUpdateRequestDTO.builder().name("News").build();
    var res = CategoryResponseDTO.builder().id(id).name("News").build();

    given(categoryService.updateCategory(eq(id), any(CategoryUpdateRequestDTO.class)))
        .willReturn(res);

    int s = mvc
        .perform(put("/api/categories/{id}", id).with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf())
            .contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsString(req)))
        .andReturn().getResponse().getStatus();
  ...
  @MethodSource("authedCases")
  void delete_category_role_matrix(String who, Supplier<RequestPostProcessor> auth)
      throws Exception {
    UUID id = UUID.randomUUID();
    willDoNothing().given(categoryService).deleteCategory(id);

    int s = mvc
        .perform(delete("/api/categories/{id}", id).with(auth.get())
            .with(SecurityMockMvcRequestPostProcessors.csrf()))
        .andReturn().getResponse().getStatus();
